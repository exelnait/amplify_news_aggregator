...
export type IAggregateRSSFeedInvokeInput = Partial<ScheduledEvent<IAggregateRSSFeedDetail>>

export const handler: ScheduledHandler<IAggregateRSSFeedDetail> = async (event) => {
    ...
    try {
        ...
        if (detail?.publisherID) {
            ...
            sources = await getPublisherSourcesByTypeAndPublisher(SourceType.RSS, detail.publisherID);
        } else {
