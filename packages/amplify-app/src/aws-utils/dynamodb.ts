...
function createPutItemCommandParams<I extends IDefaultInput>(
  input: I,
  type: string,
  additionalParams: Partial<PutItemCommandInput> = {}
) {
  const item = {
    __typename: type,
...
export async function putItem<T, I extends IDefaultInput>(
  tableName: string,
  input: I,
  type: string,
  additionalParams: Partial<PutItemCommandInput> = {}
...
export async function putItemUnique<T, I extends IDefaultInput>(
  tableName: string,
  input: I,
  type: string,
  conditionKey: string
...
export function putItemBatch<T, I extends IDefaultInput>(
  tableName: string,
  inputs: I[],
  type: string,
  additionalParams: { ConditionExpression?: string } = {}
): Promise<T[]> {
  const items = inputs.map((input) =>
    createPutItemCommandParams<I>(input, type, additionalParams)
  );
...
export async function putItemBatchUnique<T, I extends IDefaultInput>(
  tableName: string,
  inputs: I[],
  type: string,
  conditionKey: string
  ...
  return Promise.all(
    batchesArray.map((b) => {
      ...
      return docClient.send(
        new BatchWriteItemCommand({ RequestItems: currentParams })
      );
    })
  );
};
