 ...
 * @param options
 */
export function parseSrcset(string: string, options?: Options): SrcSetDefinition[] {
    ...
    return string.split(imageCandidateRegex)
        ...
        .map(part => {
            ...
            const result: SrcSetDefinition = {url};
 ...
 * @param options
 */
export function stringifySrcset(array: readonly SrcSetDefinition[], options: Options = {}): string {
    const {strict = false} = options || {};
