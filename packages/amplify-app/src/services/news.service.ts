...
import {
  calculateReadingTimeInMillisecondsByWordsCount,
  getDomain,
} from '../utils/rss/rss_utils';
...
export async function normalizeNewsItemFromRSS(
  item: NewsItem,
  options: NormalizeNewsItemOptions = {
  ...
  try {
    ...
    item.rss.contentText =
      scrapedArticle?.text || parsedArticle.content
